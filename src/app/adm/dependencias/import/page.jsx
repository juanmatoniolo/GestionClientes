...
export default function ImportDependenciasPage() {
    ...
    const parse = () => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (/Azul/i.test(L) && !/Profesor/.test(L)) {
                ciudadGlobal = "Azul";
                if (current) current.ciudad = ciudadGlobal;
                continue;
            }
    ...
    return (
        <main className="container py-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <h1 className="m-0">Importar Dependencias</h1>
            ...
            <p className="text-muted">
                Pegá el texto completo y parseamos automáticamente a “juzgados” con sus secretarías.
            </p>
            ...
            <div className="mb-3">
                ...
                <textarea
                    ...
                    placeholder="Pegá aquí el texto..."
                />
            </div>

            <div className="d-flex gap-2 mb-3">
                <button className="btn btn-outline-primary" onClick={parse} disabled={!raw.trim()}>
                    Previsualizar
                </button>
                <button className="btn btn-success" onClick={upload} disabled={!preview.length}>
                    Subir a Firebase
                </button>
            </div>
                                    ...
                                    <td>
                                        {j.secretarias?.map((s, k) => (
                                            <div key={k} className="mb-2">
                                                <div>
                                                    <strong>{s.rotulo}</strong> {s.numero ? `(Nro. ${s.numero})` : ""}
                                                </div>
                                                {s.telefonos?.length ? (
                                                    <div className="small">Tel: {s.telefonos.join(" / ")}</div>
                                                ) : null}
                                                {s.emails?.length ? (
                                                    <div className="small">Email: {s.emails.join(" / ")}</div>
                                                ) : null}
                                            </div>
                                        ))}
                                    </td>
